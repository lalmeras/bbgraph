...
create table ${schema}.Announcement (id int8 not null, active boolean not null, creation_date timestamp not null, creation_subject_label ${type.text}, creation_subject_reference_id int8, creation_subject_reference_type varchar(255), creation_subject_serialized ${type.text}, description_en ${type.text}, description_fr ${type.text}, interruption_endDateTime timestamp, interruption_startDateTime timestamp, modification_date timestamp not null, modification_subject_label ${type.text}, modification_subject_reference_id int8, modification_subject_reference_type varchar(255), modification_subject_serialized ${type.text}, publication_endDateTime timestamp not null, publication_startDateTime timestamp not null, title_en ${type.text}, title_fr ${type.text}, type varchar(255) not null, primary key (id));
...
create table ${schema}.user_ (id int8 not null, active boolean not null, creationDate timestamp not null, lastLoginDate timestamp, lastUpdateDate timestamp not null, locale varchar(255), passwordHash ${type.text}, username ${type.text} not null, email ${type.text}, faxNumber ${type.text}, firstName ${type.text} not null, gsmNumber ${type.text}, lastName ${type.text} not null, phoneNumber ${type.text}, announcementInformation_lastActionDate timestamp, announcementInformation_open boolean not null, passwordInformation_lastUpdateDate timestamp, passwordRecoveryRequest_creationDate timestamp, passwordRecoveryRequest_initiator varchar(255), passwordRecoveryRequest_token ${type.text}, passwordRecoveryRequest_type varchar(255), primary key (id));
