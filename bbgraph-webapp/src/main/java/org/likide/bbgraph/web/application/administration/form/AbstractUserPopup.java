...
import org.likide.bbgraph.web.application.common.validator.UsernamePatternValidator;
				...
				protected void onSubmit(AjaxRequestTarget target) {
					try {
						...
						if (addModeCondition().applies()) {
							...
							userService.onCreate(user, BbgraphSession.get().getUser());
							
							if (StringUtils.hasText(password)) {
								securityManagementService.updatePassword(user, password, BbgraphSession.get().getUser());
							} else {
							...
							User authenticatedUser = BbgraphSession.get().getUser();
							if (authenticatedUser != null && authenticatedUser.equals(user) && user.getLocale() != null) {
								BbgraphSession.get().setLocale(user.getLocale());
							}
