package org.likide.bbgraph.web.application.common.template;
...
import org.likide.bbgraph.web.application.common.component.AnnouncementsPanel;
import org.likide.bbgraph.web.application.common.component.ApplicationEnvironmentPanel;
import org.likide.bbgraph.web.application.common.template.theme.BbgraphApplicationTheme;
import org.likide.bbgraph.web.application.common.template.theme.common.BootstrapBreakpointPanel;
...
public abstract class MainTemplate extends AbstractWebPageTemplate {
	...
	private final IModel<BbgraphApplicationTheme> applicationThemeModel = ApplicationPropertyModel.of(APPLICATION_THEME);
	...
	public MainTemplate(PageParameters parameters) {
		...
		if (securityManagementService.isPasswordExpired(BbgraphSession.get().getUser())) {
		...
		add(
			new TransparentWebMarkupContainer("htmlElement")
				.add(AttributeAppender.append("lang", BbgraphSession.get().getLocale().getLanguage()))
		);
		
		add(
			new TransparentWebMarkupContainer("bodyElement")
				.add(new ClassAttributeAppender(BbgraphSession.get().getEnvironmentModel()))
		);
	...
	protected List<NavigationMenuItem> getMainNav() {
		return ImmutableList.of(
			...
			LinkDescriptorBuilder.start()
				.validator(Condition.role(BbgraphAuthorityConstants.ROLE_ADMIN))
	...
	protected BbgraphApplicationTheme getApplicationTheme() {
